...
| **C. Automate prep + PR creation; auto-tag via GH Actions after merge (CHOSEN)** | One command from "nothing" to "PR open"; keeps CEO approval as the sole human gate; auto-tag fires AFTER merge so it always lands on the squash commit (never the branch HEAD); reusable GH Actions workflow in `golden-paths/pipelines/` for adopters who want the same pattern on managed projects | Adds a GH Actions workflow; requires `contents: write` permission (already granted by default `GITHUB_TOKEN` on all GH repos) |
...
Chosen: **Option C** — automate the mechanical prep (changelog generation + CHANGELOG.md update + release-branch push + PR creation) in the `/release` skill, and add a GH Actions workflow that auto-tags the squash commit when the release PR merges to `main`. Keep the CEO approval gate as the only required human step.
...
5. Creates a GitHub Release entry using the CHANGELOG section extracted from the PR body.
...
- `.github/workflows/auto-tag-on-release-pr-merge.yml` — the fork's own auto-tag workflow for apexyard releases.
